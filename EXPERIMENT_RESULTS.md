...
All LoCoMo optimization rows below use the train split with 80 examples. Test
rows use the full test split with 1,449 examples. Progressive and bandit runs
use the docker proposer sandbox. Default runs predate the docker sandbox and
are kept as baselines. The `bandit v2` column is the newer z-score reward
bandit run from 2026-04-28. The opus bandit v2 test result is intentionally
excluded because that test artifact was deleted.

### Train

| proposer family | default | progressive (docker) | bandit (docker) | bandit v2 (docker) | bandit v3 (docker) | best |
|---|---:|---:|---:|---:|---:|---|
| claudekimi | 0.4000 | 0.4375 | 0.4125 | **0.4500** | 0.4375 | bandit v2 |
| claude opus | 0.3875 | **0.4750** | 0.4125 | N/A | N/A | progressive |
| codex54 | 0.4125 | 0.4250 | 0.3750 | **0.4625** | 0.4250 (iter28/30) | bandit v2 |

### Test

Test rows include the v3 result inline so each (proposer family, policy) cell
holds a single number. The "best" column reflects the proposer family's
strongest policy across default/progressive/bandit/bandit v2/bandit v3.
...
Bandit v3 introduces a sliding-window z-score reward (window=16) that rewards
each iteration relative to recent quality history rather than absolute
improvement over the run's best. The "passrate-reward" variant restricts the
reward signal to passrate (excluding average_score) so the bandit does not
pursue partial-quality wins that don't carry to test.

Train rows use the same full80 seed as v2. Test rows use the full 1,449
example test split. All three v3 runs use the docker proposer sandbox.

| proposer family | variant | iters completed | best train | best train candidate | test |
|---|---|---:|---:|---|---:|
| claudekimi | v3 (passrate-only reward) | 30/30 | **0.4375** | iter022_memgpt_temporal_resolved_v1 | **0.3589** |
| codex54 | v3 (z-score, mixed reward) | 27/30 | 0.4250 | iter012_memgpt_support_chains_v12 | **0.3865** |

Notes:

- claudekimi v3 keeps only the passrate-only reward variant; an earlier
  mixed-reward run was inferior on every metric (train 0.3750 / test 0.3009)
  and is dropped. The mixed-reward result confirmed that putting average_score
  in the reward chases partial-quality wins that do not transfer to test.
- The passrate-only v3 result matches the older claudekimi bandit run on test
  (0.3589 ≈ 0.3616) but still trails progressive at 0.3734.
- codex54 v3 is the strongest codex54 result on LoCoMo test (0.3865), beating
  codex54 progressive (0.3589) and codex54 default (0.3471). This is the only
  bandit-family run that beats progressive on this benchmark for any proposer
  family. The train run was interrupted at iter28/30; test was evaluated on
  2026-04-30 against the full 1,449-example test split using Qwen3-8B on
  GPU1 (port 8002, 128 workers).

Notes (v1/v2):

- The overall LoCoMo test best is **claude opus progressive docker at 0.3982**.
- Progressive wins for claudekimi and claude opus on test. Codex54 is the
  exception: bandit v3 (0.3865) is the strongest codex54 result on test, the
...
- claudekimi bandit v2 improves train (0.4500) but not test (0.3395), so it is
  another train-test mismatch.
- The deleted opus bandit v2 test result is not counted in this table.

### Proposer cost

Per-iteration averages over `proposer_calls/*/agent/metrics.json`. `input` and
`output` are the new tokens billed each turn; `cache read` is the prompt-cache
hit (workspace context reused across turns) which is much larger but billed at
a fraction of new-input rate. `tools/iter` is total tool-use calls per
iteration; `files/iter` is unique files the proposer opened per iteration.

| proposer (policy) | iters | input/iter | output/iter | cache read/iter | tools/iter | files/iter | dur/iter |
|---|---:|---:|---:|---:|---:|---:|---:|
| claudekimi progressive | 29 | 138.9k | 25.5k | 1.70M | 35.2 | 15.1 | 13.0m |
| claudekimi bandit v3 (passrate-only) | 30 | 104.2k | 29.8k | 1.83M | 35.1 | 17.6 | 14.1m |
| opus progressive | 30 | 3.1k | 20.6k | 1.99M | 61.2 | 20.7 | 8.9m |
| codex54 progressive | 30 | **2.39M** | 18.7k | 2.25M | 50.6 | 16.9 | 7.1m |
| codex54 bandit v3 | 27 | 1.13M | 20.7k | 995k | 34.6 | 18.5 | 7.0m |

Default LoCoMo rows are omitted because the train-run directories were deleted;
only the test-eval dirs remain. Codex54's input-token cost is roughly an order
of magnitude higher than claudekimi/opus because gpt-5.4 expands reasoning
inline; v3 cuts it nearly in half (2.39M → 1.13M) without losing test quality.
The opus proposer keeps almost everything in the prompt cache (input ≈ 3k),
so its real cost is dominated by cache reads.
...
Train rows use train100. Test rows use the 400-example test split. Rows marked
`failed` produced a test-frontier artifact but did not complete a valid score.
The opus46 progressive test was retried and stopped after hanging; its last
completed status is a Together judge 500 error.

### Train

| proposer family | default | progressive | bandit | best |
|---|---:|---:|---:|---|
| claude opus46 | N/A | **0.6300** | N/A | progressive |
| claudekimi | 0.5600 | **0.6000** | 0.5700 | progressive |
| codex54 | **0.6000** | 0.5400 | 0.5700 | default |
...
- Fresh LongMemEval test-frontier runs completed for claudekimi progressive,
  claudekimi bandit v2, codex54 default, codex54 progressive rerun, and codex54
  bandit v2 on 2026-04-29.
- The best current LongMemEval test result is claudekimi progressive at 0.5000.
  Codex54 default is second at 0.4875.
- Bandit v2 does not beat default/progressive on LongMemEval test: claudekimi
  bandit v2 is 0.4400 and codex54 bandit v2 is 0.4575.
...
- Opus46 progressive is strongest on train100 at 0.6300, but its test-frontier
  attempt failed with a Together 500; a rerun was stopped before completion.

### Proposer cost

Per-iteration averages over `proposer_calls/*/agent/metrics.json`. No bandit
v3 run was attempted on LongMemEval, so the bandit column is omitted.

| proposer (policy) | iters | input/iter | output/iter | cache read/iter | tools/iter | files/iter | dur/iter |
|---|---:|---:|---:|---:|---:|---:|---:|
| claudekimi default | 30 | 121.5k | 26.9k | 2.12M | 39.6 | 18.4 | 10.3m |
| claudekimi progressive | 30 | 105.0k | 25.0k | 1.73M | 33.6 | 16.3 | 9.5m |
| opus46 progressive | 30 | 1.7k | 17.3k | 1.48M | 61.3 | 20.0 | 7.2m |
| codex54 default | 30 | **1.77M** | 27.4k | 1.61M | 33.4 | 18.8 | 9.5m |
| codex54 progressive (rerun) | 30 | 1.45M | 25.0k | 1.33M | 31.9 | 17.0 | 8.3m |

The pattern matches LoCoMo: codex54 dominates input-token cost, opus rides on
the prompt cache, and claudekimi sits in between. Files-per-iter (~17–20) and
tools-per-iter (~32–61) are similar across benchmarks.
...
Conclusion: there is no clean bandit comparison yet. Default is stronger in the
newer claudekimi offline validation run; older test reruns are close and do not
show a consistent progressive advantage.
...
| policy | source baseline | best optimizer candidate | best passrate | iters completed |
|---|---:|---|---:|---:|
...
- Progressive is the strongest current SWE-bench result. The rerun's lower
  ...
  rerun's own baseline (0.4 → 0.5333) and the earlier baseline (0.4667 →
  0.5333).
- `iter016` introduces a fallback traceback-aware retrieval; the next four
  ...
  this pool.
- Default was also interrupted before reaching iter30 but still produced five
  0.5000 candidates with rate-of-discovery similar to the rerun, so the
  current "progressive > default" gap on train30 is plausibly within
  run-to-run noise.

### Proposer cost (mimo v2.5 train30)

Per-iteration averages over `proposer_calls/*/agent/metrics.json`. No bandit
v3 run was attempted on SWE-bench, so the bandit column is omitted.

| proposer (policy) | iters | input/iter | output/iter | cache read/iter | tools/iter | files/iter | dur/iter |
|---|---:|---:|---:|---:|---:|---:|---:|
| claudekimi default | 20 | 136.3k | 28.6k | 3.06M | 56.0 | 23.2 | 13.4m |
| claudekimi progressive | 20 | 141.8k | 29.7k | 3.61M | 61.0 | 23.6 | 12.5m |

Tools/iter (56–61) and files/iter (~23) are higher than on LoCoMo/LongMemEval
because the SWE-bench workspace is larger (full mini-SWE-agent source plus
...
- LoCoMo claudekimi bandit v3 passrate reward improves over bandit v2 on test
  (0.3589 vs. 0.3395), but remains below progressive (0.3734).
